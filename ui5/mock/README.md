...
- `products.json` — `ProductCollection` (123 products, upstream order) and
  ...
  `src/01/z2ui5_cl_dmo_app_033.clas.abap`, and the name/quantity rows of
  `src/01/z2ui5_cl_dmo_app_030.clas.abap`; the stats `SupplierName` group
...
- `countriesExtendedCollection.json` — `CountriesCollection` (70 countries),
  ...
  `src/01/z2ui5_cl_dmo_app_011.clas.abap`. Used by the ComboBox samples.
...
- `supplier.json` — `SupplierCollection` (one record, Red Point Stores).
  ...
  (`src/01/z2ui5_cl_dmo_app_020.clas.abap`, six bound columns).
