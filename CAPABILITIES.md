...
| MessageManager auto-collection (the global `message>` model gathering **control validation** messages client-side) | ❌ | not needed to render a MessageView (row above); for validation, drive the message table from ABAP | app 449 renders the static message set 1:1 without it |
