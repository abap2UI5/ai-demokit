...
| Controller-created popup-mode control opened imperatively (`new PDFViewer(...)` + `.open()` on a view dependent) | ✅ | declare the control in the view's `mvc:dependents` aggregation (the `addDependent` equivalent), bind its inputs, open it with `client->follow_up_action( val = cs_event-control_by_id t_arg = id/``/'open' )` after the round-trip - `open`/`close` are whitelisted since 2026-07-18 (pr/control-call-whitelist) | app 469 (LIVE-TEST pending); source-verified in `app/webapp/core/FrontendAction.js` `CONTROL_METHODS` |
...
| UI5 feature | Status | How in abap2UI5 | Evidence |
|---|---|---|---|
| JSON model data | ✅ | ABAP `VALUE #( … )` + `client->_bind` (two-way; never the obsolete `_bind_edit`) — keep the data verbatim, full row set | AGENTS §5; silent row subsets bit apps 433/440/441 |
| Two-way property binding incl. live client-side updates | ✅ | shared `_bind` path on both controls — no event round-trip needed | app 527 (Switch↔Select), app 530 (Breadcrumbs separatorStyle↔Select) |
| Expression bindings (`{= … }`) over bound paths | ✅ | inline `client->_bind( var )` calls composed into the expression string template — never captured in a variable | app 421 (the AGENTS §5 worked example) |
...
| Binding `sorter` (no grouping) | ✅ | keep the original binding-info string: `\|\{ path: '{ client->_bind( val = t path = abap_true ) }', sorter: \{ path: 'COL' \} \}\|` — no ABAP SORT, no deviation needed | apps 423/440/527 converted 2026-07-17; same pass-through as the group-sorter row below |
...
| Imperative one-shot control methods with no binding equivalent (`open [pageKey]`, `close`, `setExpanded`, `scrollToIndex`, `scrollTo`, `focus`, NavContainer `to [transitionName]`/`back`, Wizard `goToStep`/`discardProgress`/`setNextStep`) | ✅ | `client->follow_up_action( val = cs_event-control_by_id t_arg = id/view/method/params… )` — whitelisted per method with arg-kind casting (`bool` takes the ABAP `X`/space), runs client-side after render; `open`/`close`/`setExpanded` added 2026-07-18 (pr/control-call-whitelist), optional args for `to`/`open` + `goToStep` added 2026-07-19 (pr/control-method-args). **Arguments beyond the method's declared kinds are still silently dropped** — check `CONTROL_METHODS` BEFORE wiring a parametrized call; a needed-but-unlisted arg is a declared deviation + pr/ request, never a LIVE_TEST hope (probe apps 609/624, 2026-07-19) | apps 469 (`open`) and 471 (`setExpanded`), LIVE-TEST pending; source-verified in `app/webapp/core/FrontendAction.js` `CONTROL_METHODS` |
...
pr/control-call-whitelist — and since 2026-07-19 `goToStep` plus the
optional `to`/`open` args — pr/control-method-args),
...
id/view/method/params, `control_global`: object/method/params,
`binding_call`: id/aggregation/method/params), or roundtrip-free via
