...
| Imperative one-shot control methods with no binding equivalent (`open [pageKey]`, `close`, `setExpanded`, `scrollToIndex`, `scrollTo`, `focus`, NavContainer `to [transitionName]`/`back`, Wizard `goToStep`/`discardProgress`/`setNextStep`, `openBy [anchorId]`, Carousel `setActivePage`) | ✅ | `client->follow_up_action( val = cs_event-control_by_id t_arg = id/view/method/params… )` — whitelisted per method with arg-kind casting (`bool` takes the ABAP `X`/space, `domRef` resolves a control id to its DOM element with control fallback), runs client-side after render; `open`/`close`/`setExpanded` added 2026-07-18 (pr/control-call-whitelist), optional args for `to`/`open` + `goToStep` 2026-07-19 (pr/control-method-args), `openBy`/`setActivePage` 2026-07-20 (pr/control-methods-openby-setactivepage). **Arguments beyond the method's declared kinds are still silently dropped** — check `CONTROL_METHODS` BEFORE wiring a parametrized call; a needed-but-unlisted arg is a declared deviation + pr/ request, never a LIVE_TEST hope (probe apps 609/624, 2026-07-19). Aggregation-template **clones** (bound pages/items) have no backend-addressable ids — a method arg can only name statically declared controls (app 536's Carousel re-sync limit) | apps 469 (`open`), 471 (`setExpanded`), 540 (`openBy`), LIVE-TEST pending; source-verified in `app/webapp/core/FrontendAction.js` `CONTROL_METHODS` |
