...
| Custom JS formatter **functions** (`Formatter.js`, a `formatter: '.fn'` controller method) | 🔶 | when the logic fits the expression parser's whitelisted globals (`parseFloat`, `parseInt`, `isNaN`, `Math`, `Number`, `String`, …): reproduce it as a CSP-safe `{= … }` expression binding - behavior-identical, no eval. Otherwise preformat in ABAP and bind the result; note it. **Never via runtime-compiled JS strings**: an eval-based `register_formatter` was implemented and reverted the same day (2026-07-18, security decision - needs unsafe-eval, invites building code from data); pr/formatter-registry stays open for a CSP-clean served-module design. Factories returning controls (`groupHeaderFactory`, item factories) are ❌ either way | app 401 reproduces `weightState` (NaN/negative→None, G→/1000, <1/<5 thresholds) as an expression binding (LIVE-TEST pending) |
