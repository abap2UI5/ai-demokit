 ...
 * deploy.
 *
 * NOT committed — written into web/search/, beside the three static files it
 * feeds, and gitignored there. `deploy-web` regenerates it on every deploy, so
 * it is never staler than the site serving it, and a port PR does not carry a
 * 350 KB diff of derived data.
 *
 ...
 * gets a DISABLED playground button naming what is missing, rather than a
 * link to a blank frame. The version is what that build pins — a port needing
 * a release above it would not render either, and the page says so.
 *
 * This is a COPY of a constant in another repository, and deliberately so: it
 * can only go stale in the safe direction. A library added there and not here
 * greys out a button that would have worked; the reverse — a link that fails —
 * would need a library to be REMOVED there, which is not a thing that happens
 * quietly. Update it after the playground deploys, never before. */
const PLAYGROUND_LIBS = new Set([
  'sap.ui.core', 'sap.m', 'sap.f', 'sap.ui.layout',
  'sap.ui.table', 'sap.ui.unified', 'sap.tnt', 'sap.uxap',
  /* added to that build 2026-08-19 (playground #12) for this catalogue: the
   * integration cards and the code editor were the only two grey libraries
   * that OpenUI5 ships at all. */
