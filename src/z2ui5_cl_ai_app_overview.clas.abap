    ...
    result = VALUE #(
      ...
      ( module = `sap.m` control = `sap.m.FacetFilter`     name = `FacetFilterLight`          class = `z2ui5_cl_ai_app_401` path = `src/01/b04/z2ui5_cl_ai_app_401.clas.abap`
        notes = `NOTE: the bound lists="{/ProductCollectionStats/Filters}" collection is represented as two static FacetFilterLists (Category, SupplierName) - the original's stats model yields exactly these two lists,` &&
                 ...
                 ` reproduced behavior-identically as a CSP-safe expression binding on the ObjectNumber state (same thresholds: NaN/negative -> None, unit G -> /1000, <1 Success, <5 Warning, else Error) - the` &&
                 ` expression parser whitelists parseFloat/isNaN, no eval involved. The earlier precomputed WEIGHT_STATE column is gone; the formatter FUNCTION mechanism itself stays inexpressible (an eval-based` &&
                 ` register_formatter was implemented and reverted the same day for security, see pr/formatter-registry - open). // LIVE-TEST: confirm the weight states render Success/Warning/Error per row via the` &&
                 ` expression binding (converted from the precomputed column 2026-07-18). // IMPROVISED: the appended table's header toolbar keeps only Title and ToolbarSpacer - the sample's popin-layout ComboBox (with` &&
