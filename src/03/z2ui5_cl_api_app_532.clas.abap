...
CLASS z2ui5_cl_api_app_532 IMPLEMENTATION.

  METHOD view_display.

    DATA(page) = z2ui5_cl_xml_view=>factory( ).

    DATA(object_page_layout) = page->object_page_layout( subsectionlayout         = `TitleOnLeft`
    ...
    " sap.m.Avatar is only available since UI5 1.73 - a plain icon is used instead
    header_title->snapped_heading(
        )->flex_box( fitcontainer = abap_true
                     alignitems   = `Center`
            )->icon( src   = `sap-icon://picture`
    ...
    " sap.m.Avatar is only available since UI5 1.73 - a plain icon is used instead
    flex_box->icon( src   = `sap-icon://picture`
                    size  = `2.5rem`
                    class = `sapUiSmallMarginEnd` ).

    flex_box->vertical_layout(
    ...
    flex_box->vertical_layout( class = `sapUiSmallMarginBeginEnd`
        )->label( `Hello! I am Tim and I use UxAP`
        )->vbox(
  ...
  METHOD render_goals_section.
    ...
    sections->object_page_section( titleuppercase = abap_false
            ...
            )->object_page_sub_section( title          = `Goal summary`
  ...
  METHOD render_personal_section.
    ...
    " shared blocks BlockAdresses, BlockPhoneNumber, BlockSocial and BlockMailing rebuilt as static form content
    ...
    DATA(payment) = sub_sections->object_page_sub_section( title          = `Payment information`
                                                           titleuppercase = abap_false ).
  ...
  METHOD render_employment_section.
    ...
    DATA(job_info_blocks) = sub_sections->object_page_sub_section( title          = `Job information`
                                                                   titleuppercase = abap_false
        )->blocks( ).
    ...
    DATA(details) = sub_sections->object_page_sub_section( title          = `Employee Details`
                                                           titleuppercase = abap_false ).
    ...
    DATA(job_grid) = sub_sections->object_page_sub_section( title          = `Job Relationship`
                                                            titleuppercase = abap_false
        )->blocks(
