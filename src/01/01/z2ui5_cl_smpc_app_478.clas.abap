...
CLASS z2ui5_cl_smpc_app_478 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    view->ele( n = `View` ns = `mvc`
                ...
                )->a( n = `text`     v = `MultiInput with "start with" filtering: enter a search term, e.g. "A", and see filtered list`
                )->a( n = `width`    v = `100%`
                ...
                )->a( n = `text`     v = `MultiInput with "contains" filtering: enter a search term, e.g. "40", and see filtered list`
                )->a( n = `width`    v = `100%`
  ...
  METHOD model_init.
    ...
    DATA temp1 TYPE z2ui5_cl_smpc_app_478=>ty_t_product.
