" @keywords icontabbar sap.m icontabbarresponsivepadding responsivepadding icontabfilter icontabseparator showall count table filter
  ...
  METHOD filter_apply.
    ...
    LOOP AT t_products INTO DATA(row).
      DATA(kg) = COND decfloat34( WHEN row-weightunit = `G`
                                  ...
                                  ELSE CONV decfloat34( row-weightmeasure ) ).
      DATA(keep) = SWITCH abap_bool( key
                                     WHEN `Ok`         THEN xsdbool( kg < 1 )
                                     WHEN `Heavy`      THEN xsdbool( kg >= 1 AND kg <= 5 )
