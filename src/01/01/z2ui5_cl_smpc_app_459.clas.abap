...
CLASS z2ui5_cl_smpc_app_459 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    view->ele( n = `View` ns = `mvc`
                ...
                )->a( n = `text`     v = `MultiComboBox with "starts with per term" filtering: enter a search term, e.g. "N", and see filtering results.`
                )->a( n = `labelFor` v = `multiCombo1`
            )->ele( `MultiComboBox`
                )->a( n = `id`       v = `multiCombo1`
                )->a( n = `maxWidth` v = `650px`
  ...
  METHOD model_init.
    ...
    DATA temp1 TYPE z2ui5_cl_smpc_app_459=>ty_t_product.
