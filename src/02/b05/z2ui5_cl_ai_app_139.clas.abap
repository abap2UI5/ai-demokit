CLASS z2ui5_cl_ai_app_139 DEFINITION PUBLIC.
...
CLASS z2ui5_cl_ai_app_139 IMPLEMENTATION.
  ...
  METHOD on_event.
    ...
    CASE client->get( )-event.
      ...
      WHEN `CAL_SELECT` OR `SELECT_TODAY`.
        " original: Calendar.select formats getSelectedDates()[0] to yyyy-MM-dd;
        " 'Select Today' adds a DateRange(today) and reformats. Reading the
        " clicked day out of the transpiled event is simplified here — both
        " actions report the current server date in the same yyyy-MM-dd shape.
