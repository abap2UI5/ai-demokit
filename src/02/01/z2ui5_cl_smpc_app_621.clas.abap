...
CLASS z2ui5_cl_smpc_app_621 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    layout->tag( `Label`
        )->a( n = `text`     v = `Tabular suggestions with default configuration.`
    ...
    layout->tag( `Label`
        )->a( n = `text`     v = `Tabular suggestions with enableTableAutoPopinMode="true"`
        )->a( n = `labelFor` v = `popinTableInput`

        )->ele( `Input`
            )->a( n = `id`                           v = `popinTableInput`
            )->a( n = `placeholder`                  v = `Enter product`
            )->a( n = `enableTableAutoPopinMode`     v = `true`
            )->a( n = `showSuggestion`               v = `true`
            )->a( n = `showTableSuggestionValueHelp` v = `false`
            )->a( n = `suggestionRows`               v = client->_bind( t_products )
    ...
    layout->tag( `Label`
        )->a( n = `text`     v = `Tabular suggestions with custom column popin configuration`
        )->a( n = `labelFor` v = `customPopinTableInput`

        )->ele( `Input`
            )->a( n = `id`                           v = `customPopinTableInput`
            )->a( n = `placeholder`                  v = `Enter product`
            )->a( n = `showSuggestion`               v = `true`
            )->a( n = `showTableSuggestionValueHelp` v = `false`
            )->a( n = `suggestionRows`               v = client->_bind( t_products )
  ...
  METHOD model_init.
    ...
    " sap/ui/demo/mock/products.json /ProductCollection - the suggestion rows
    DATA temp1 TYPE z2ui5_cl_smpc_app_621=>ty_t_product.
