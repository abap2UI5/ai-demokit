" @keywords multicombobox multi combo box sap.m multicomboboxmaxpickerheight label
...
CLASS z2ui5_cl_smpc_app_495 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    view->ele( n = `View` ns = `mvc`
                        ...
                        )->a( n = `text`     v = `MultiComboBox without maxPickerHeight`
                        )->a( n = `labelFor` v = `idMultiComboBoxDefault`
                    )->ele( `MultiComboBox`
                        )->a( n = `class` v = `sapUiSmallMarginBottom`
                        )->a( n = `id`    v = `idMultiComboBoxDefault`
