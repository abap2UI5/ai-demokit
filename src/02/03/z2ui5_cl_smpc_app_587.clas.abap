" @keywords objectpagelayout object layout sap.uxap anchorbarwithnumbers objectpagedynamicheadertitle title flexbox avatar text button overflowtoolbarbutton
...
CLASS z2ui5_cl_smpc_app_587 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    " Block->content inlining (app 401/416 precedent) plus the named-model fold of
    " app 230: the blocks aggregations hold the same SharedBlocks BlockBase
    " controls app 401 carries, each a lazy-loading wrapper around a static view -
    " inlined 1:1 below. The EmploymentBlockJob block additionally carries
    " uxap:ModelMapping elements mapping ObjectPageModel>/Employee/N onto internal
    " models empN>; abap2UI5 serves one default model, so those fold onto one
    " table addressed per row.
