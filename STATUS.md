...
| Deviations | 4 DROPPED_171 · 138 IMPROVISED · 67 LIVE_TEST · 275 NOTE · 95 POST_171 |
| Open LIVE_TESTs | **63 ports** carry at least one `LIVE_TEST` deviation — the automated close path is the e2e interaction harness (AGENTS §6 `e2e_smoke`) |
...
- [ ] **Review-sweep rework backlog (42 ports left).** The 2026-07-27 sweep
  ...
  (a sample-local helper module) stay dropped, now declared as such. **Still
  open:** toast substitutions around capabilities CAPABILITIES marks
  expressible (MessageBox `onclose`, `popover_display`, URLHELPER, timers,
  generalized `control_by_id` methods — the app-042 class), faked event values
  where the `$event.*` transport exists, and dropped sample CSS (122/124, plus
  §4 archive gaps). Find them: sidecar status `generated` minus the 6
  scope-exception ports. Note the six reworked ports keep status `generated`:
  the headline gap is closed and gate-verified, a full end-to-end re-review
  per port is not done.
