...
export default async (page, expect) => {
  ...
  // the two relative views are declared on the calendar
  await waitForUi5(page, () => ui5All().filter((c) => c.getMetadata().getName() === 'sap.m.PlanningCalendarView')
    .filter((c) => c.getRelative() === true).length === 2, 'the two relative views never rendered');
};
