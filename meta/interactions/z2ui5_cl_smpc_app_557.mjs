...
export default async (page, expect) => {
  // lists="{/T_FILTERS}" has to produce the two groups of the stats model
  await waitForUi5(page, () => ui5All().filter((c) => c.getMetadata().getName() === 'sap.m.FacetFilterList').length === 2,
    'the bound lists aggregation did not produce the two facet groups');
  await waitForUi5(page, () => {
    const titles = ui5All().filter((c) => c.getMetadata().getName() === 'sap.m.FacetFilterList').map((c) => c.getTitle());
  ...
  await waitForUi5(page, () => {
    const lists = ui5All().filter((c) => c.getMetadata().getName() === 'sap.m.FacetFilterList');
    const cat = lists.find((c) => c.getTitle() === 'Category');
    return cat && cat.getItems().length === 16;
  }, 'the nested values table never reached the Category list');
  await waitForUi5(page, () => {
    const lists = ui5All().filter((c) => c.getMetadata().getName() === 'sap.m.FacetFilterList');
    const sup = lists.find((c) => c.getTitle() === 'SupplierName');
