...
export default async (page, expect) => {
  ...
  // zoomIn steps the bound scaleFactor
  await waitForUi5(page, () => ui5All().some((c) => c.getMetadata().getName() === 'sap.m.SinglePlanningCalendar'
    && c.getScaleFactor() === 1), 'the calendar did not boot at the default scale factor');
  await page.evaluate(`(() => {
    const ui5All = () => Object.values(sap.ui.require("sap/ui/core/Element").registry.all());
    ui5All().find((c) => c.getMetadata().getName() === 'sap.m.Button' && c.getIcon() === 'sap-icon://zoom-in').firePress();
  })()`);
  await waitForUi5(page, () => ui5All().some((c) => c.getMetadata().getName() === 'sap.m.SinglePlanningCalendar'
    && c.getScaleFactor() === 2), 'the zoom-in press never reached the bound scale factor');
};
