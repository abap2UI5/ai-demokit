...
| [`unified-menu-open-anchored`](unified-menu-open-anchored/) | Anchored open for `sap.ui.unified.Menu`, which — unlike `sap.m.Menu` — has **no `openBy`**, only `open(kbd, opener, my, at, of)` (self-closes when `of` is absent). The whitelisted `openBy`/`open` both fail on it, so the button-press open is a **no-op today**. Proposed: fall back to `open(false, anchor, "begin top", "begin bottom", anchor)` when the target lacks `openBy`. From b10 apps 227/228; blocks the whole `sap.ui.unified.Menu` sample family. | medium |
| [`scope-since-from-source`](scope-since-from-source/) | The out-of-scope gate is **blind offline**: `universe.json.since` is null (no `api.json` in the fork), so `scopeOf` treats every unknown-since control as in-scope — that is how `sap.f.AvatarGroup` (@1.73) nearly slipped in and `sap.f.SidePanel` (@1.107, app 136) already did. `scripts/scope-of.mjs` now reports the authoritative verdict from source `@since`/`@deprecated`; open ask is to wire `scopeOf` to it (and reclassify app 136). From apps 230–235. | medium |
...
| [`feedinput-enable-post-button`](feedinput-enable-post-button/) | Whitelist `sap.m.FeedInput.enablePostButton [bool]` in `CONTROL_METHODS` — a public method with no bindable-property equivalent (`enabled` disables the whole input, not just the Post button). From app 236. | low |
...
| [`nav-app-call-caller-draft`](nav-app-call-caller-draft/) | Browser Back after a `nav_app_call` restores the CALLER as it was *rendered*, dropping every client-side change the user made before navigating (the overview's Shell/Tree switches, its filter checkboxes). The response now carries `nav_app_call_prev_app`/`_id` — the draft the caller was just saved under — and `View1._repointCallerEntry` repoints the caller's history entry at it before pushing the callee's route. Branch `claude/ai-demokit-state-loss-cyz42a`; 1 ABAP unit test. | overview app |
