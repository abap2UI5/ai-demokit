...
| [`core-commandexecution-keyboard-shortcuts`](core-commandexecution-keyboard-shortcuts/) | A declarable keyboard-shortcut → named-event binding (`client->_shortcut( key event )` or a `core:CommandExecution` builder path), so `sap.ui.core.CommandExecution` samples work instead of dropping the shortcuts + faking `$cmd>…/enabled` with model flags. From app 232; the sample's entire point. | medium |
| [`formatter-date-empty-guard`](formatter-date-empty-guard/) | Make `Formatter.DateCreateObject` return `null` for a falsy input instead of `new Date('')` = **Invalid Date**. An Invalid Date is *truthy*, so a consumer that branches on the property (`sap.ui.unified` `Month._checkDateEnabled` → `CalendarDate.fromLocalJSDate`) throws and the whole view dies — an optional date in a bound row becomes a crash, not a missing value. Probe-verified (app 220: **0** days rendered vs 42). Ports can guard it themselves with an expression binding (220 does, and `pattern-lint` `unguarded-date-formatter` enforces it), so this is robustness, not a blocker. | low |
| [`event-prevent-default`](event-prevent-default/) | A declarable `preventDefault()` hook (`client->_event( … prevent_default = abap_true )`) so a client event can conditionally cancel its built-in default (e.g. `sap.tnt` `NavigationListItem.press` gating selection). From app 241; the sample's whole point. | medium |
