...
|---------|--------------|------|
| [`smartvariant-initialise`](smartvariant-initialise/) ([abap2UI5#2481](https://github.com/abap2UI5/abap2UI5/pull/2481)) | `SMART_VARIANT_INIT` — the `initialise(fnCallback, oPersoControl)` handshake `sap.ui.comp` variant management needs and that no controller-less app can make. Without it `_oPersoControl` stays `null`: saving a view dies in `sap.ui.fl` and stored views never load. Data-driven (two control ids through the slot lookup, no-op callback), waits for the smart controls to register. | app 251 |
