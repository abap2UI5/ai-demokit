...
| [`flex-enabled-for-variants`](flex-enabled-for-variants/) | `app/webapp/manifest.json` declares `"flexEnabled": false`, while SmartVariantManagement's whole purpose is persisting through the SAPUI5 flexibility back end. Saving a view in app 251 dies client-side in `_newVariant` (`Cannot read properties of undefined (reading 'getId')`) — the shape of a flex lookup that found no app component. Proposed: `flexEnabled: true` (with the two open questions in the request: it also advertises key-user adaptation, and every abap2UI5 app shares the `z2ui5` component id). Includes the console check that confirms or refutes the diagnosis. | medium |
